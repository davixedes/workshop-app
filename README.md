...
- IAM Infra Role (ECS gerencia o ALB no Blue/Green nativo)
...
- Listener Rules para produção e teste (necessárias para o Blue/Green nativo)
...
> O service vai falhar no primeiro deploy porque a imagem ainda não existe — isso é esperado. A pipeline vai corrigir isso na primeira execução.
...
```bash
curl http://SEU_ALB/orders/ORD-1234567890
```
...
3. Verifique os logs no CloudWatch: `/ecs/workshop-app` → stream `inventory/*`
   - Você deve ver linhas como `Processing order: {"id": "ORD-...", "product": "notebook", ...}`
...
| `cloudformation-infra.yml` | CloudFormation | ECR + SQS + IAM Roles + ALB + Target Groups + Listener Rules + ECS Cluster |
| `buildspec.yml` | CodeBuild | Login ECR, docker build do `orders-service`, docker push, gera `imagedefinitions.json` |
| `taskdef.json` | CodePipeline/ECS | Template da Task Definition |
...
**Build falha com "429 Too Many Requests" no Docker Hub**
→ O Dockerfile usa `FROM python:3.12-slim` sem o mirror da AWS. Troque para `FROM public.ecr.aws/docker/library/python:3.12-slim`.
